...
mainAudio.addEventListener("ended", () => {
    ...
    //current song and will do further accordingly
    ...
    switch (getText) {
        ...
        case "repeat_one": //if icon is repeat_one then repeat the same song
            mainAudio.currentTime = 0;
           loadMusic(musicIndex);
           playMusic();//calling playMusic function 
            break;

        case "shuffle": //if icon is repeat_one then change the current playing song current  playing song current time to 0 so song will play from beginning 
...
for(let i=0; i<allMusic.length; i++){
    ...
    let liTag=`<li li-index="${i + 1}">
                ...
                <span class="audio-duration ${allMusic[i].src}">3:40</span>
            </li>`;

    ulTag.insertAdjacentHTML("beforeend", liTag);
let liAudioTag = ulTag.querySelector(`#${allMusic[i].src}`);
let liAudioTagDuration = ulTag.querySelector(
    `span.${allMusic[i].src}`
);
